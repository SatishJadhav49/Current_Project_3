...
@Component({
  selector: 'app-yearlytarget',
  templateUrl: './yearlytarget.component.html',
  styleUrls: ['./yearlytarget.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class YearlytargetComponent implements AfterViewChecked {
    ...
    if (this.createStageForm.valid) {
      ...
      if (duplicate) {
        ...
        const temp: FYTarget = {
          ...
          FY_Target_L3: this.createStageForm.value.l3,
          FY_Target_L4: this.createStageForm.value.l4,
  ...
  onSave() {
    if (this.targetObj.length > 0) {
      ...
      this.targetService.saveFYTarget(temp).subscribe((data) => {
  ...
  updateStage() {
    if (this.selectedmodel.length >= 0) {
      if (this.modifyFlag) {
        const temp: FYTarget = {
          FY_Target_ID: this.selectedTargetID,
          Plant_ID: this.plantid,
          Shop_ID: this.selectedShop.Shop_ID,
          Model_ID: this.selectedmodel[0].Model_ID,
          FY_Name: this.selectYear.getFullYear(),
        ...
        this.targetService
          .updateFYTarget(this.selectedTargetID, temp)
          .subscribe((data) => {
            if (data !== null && data !== undefined) {
              if (data.isErrorMessage) {
                this.toster.error(data.messageDetail, data.messageTitle);
  ...
  DeleteRecord(id) {
    ...
    if (this.selectedForDelete) {
      this.targetService
        .deleteFYTarget(this.selectedForDelete)
        .subscribe((data) => {
  ...
  modifySelected(targetid) {
    const temp = this.targetList.find(
      (target) => target.FY_Target_ID == targetid
    );
    if (temp) {
      this.modifyFlag = true;
      this.selectedTargetID = temp.FY_Target_ID;
      this.selectYear = temp.FY_Name;
      this.selectYear = new Date(temp.FY_Name, 0, 1);
      this.createStageForm.get('l3').setValue(temp.FY_Target_L3);
      this.createStageForm.get('l4').setValue(temp.FY_Target_L4);
  ...
  LoadTable(jsondatas) {
    ...
    <any>$('#shopmodeltable').DataTable({
      ...
      columnDefs: [
        ...
        { title: 'L3 Target', targets: 3 },
        { title: 'L4 Target', targets: 4 },
        ...
        { data: 'FY_Target_L4' },
        {
          ...
          render: function (data, type, row) {
            ...
            const editButton = `
                <span id="modifymodelbtn" class="btn fa fa-pencil" data-toggle="modal" title="Edit"
                     ...
                     color: black;"
                     data-elemnt-obj="${data.FY_Target_ID}"></span>`;
            const deleteButton = `
            ...
            color: black!important;" class="btn fa fa-trash deletebutton" title = "Delete"
                     data-element-id="${data.FY_Target_ID}"></span> `;
          ...
          createdCell: (cell, cellData, rowData, rowIndex, colIndex) => {
            $(cell).on('click', '#modifymodelbtn', () => {
              this.ngZone.run(() => {
                // console.log(rowData.FY_Target_ID);
                this.modifySelected(rowData.FY_Target_ID);
              });
            });
            $(cell).on('click', '#deletemodelbtn', () => {
              this.ngZone.run(() => {
                ...
                dialogRef.afterClosed().subscribe((result) => {
                  ...
                  if (result) {
                    this.selectedForDelete = rowData.FY_Target_ID;
                    this.DeleteRecord(rowData.FY_Target_ID);
                  }
                });
              });
            });
